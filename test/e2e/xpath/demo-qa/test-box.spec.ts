...
test.describe("Xpath 001, textbox functionality", { tag: ["@smoke", "@test-box"] }, () => {
  ...
  test(`demoqa-1, test-box functionality`, { tag: ["@smoke", "@test-box"] }, async ({ page }) => {
    ...
    //labels
    const fullNameLabel = page.locator("//label[contains(@id, 'userName')]");
    const emailLabel = page.locator("//label[contains(@id, 'Email')]");
    const currentAdressLabel = page.locator("//label[contains(@id, 'current')]");
    const permanentAdressLabel = page.locator("//label[contains(@id, 'permanent')]");

    //p

    const paragraphName = page.locator("//p[contains(@id, 'name')]");
    const paragraphEmail = page.locator("//p[contains(@id, 'email')]");
    const paragraphPermanentAdreess = page.locator("//p[contains(@id, 'permanent')]");
    const paragraphCurrentAdreess = page.locator("//p[contains(@id, 'current')]");

    //inputfields && textarea

    const fullNameInput = page.locator("//input[contains(@placeholder, 'Full')]");
    const emailInput = page.locator("//input[contains(@type, 'email')]");
    const currentAdressTextArea = page.locator("//textarea[contains(@placeholder, 'Current')]");
    const permanentAdressInput = page.locator("//textarea[contains(@id, 'permanent')]");
