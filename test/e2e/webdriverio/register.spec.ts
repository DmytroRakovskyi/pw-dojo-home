...
const responsePromise = (page: Page, response: string) =>
...
test.describe("register functionality", () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect(
        registerPage.errorPanel.getByText("username is invalid"),
      ).toBeVisible();
      await expect(
        registerPage.errorPanel.getByText("email is invalid"),
      ).toBeVisible();
