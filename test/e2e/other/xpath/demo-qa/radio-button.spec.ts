...
test.describe('Xpath 003', { tag: ['@smoke', '@radio-buttons'] }, () => {
  ...
  test('demoqa-3', { tag: ['@smoke', '@radio-button'] }, async ({ page }) => {
    //headlines
    const radioButtonHeader: Locator = page.locator("//h1[contains(text(), 'Radio')]");
    const question: Locator = page.locator("//*[@class='mb-3']");

    //labels and other
    const yesRadioLabel: Locator = page.locator("//label[contains(@for, 'yes')]");
    const noRadioLabel: Locator = page.locator("//label[contains(@for, 'no')]");
    const impresRadioLabel: Locator = page.locator("//label[contains(@for, 'impressive')]");
