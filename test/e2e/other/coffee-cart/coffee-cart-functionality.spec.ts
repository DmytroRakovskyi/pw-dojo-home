...
test.describe(
  "Coffee-cart 001, main functionality",
  { tag: ["@smoke", "@smoke-coffee"] },
  () => {
    ...
    test(
      "AQA-6 empty cart can be opened",
      { tag: ["@smoke-coffee"] },
      async ({ page }) => {
        //locators
    ...
    test(
      "AQA-111 adding all elements to the cart",
      { tag: ["@smoke-coffee"] },
      async ({ page }) => {
    ...
    test(
      "AQA-8 item presence in the cart",
      { tag: ["@smoke-coffee"] },
      async ({ page }) => {
        const espresso: Locator = page.locator('[data-test="Espresso"]');
        const cartLink: Locator = page.locator("a[href*='cart']");
    ...
    test(
      ...
      async ({ page }) => {
        ...
        const plusButton: Locator = page
          ...
          .filter({ visible: true });
        const removeAllButton: Locator = page.getByRole("button", {
          name: "Remove all",
        });
    ...
    test(
      "AQA-10 remove element from cart",
      { tag: ["@smoke-coffee"] },
      async ({ page }) => {
        const espresso: Locator = page.locator('[data-test="Espresso"]');
        const cartLink: Locator = page.locator("a[href*='cart']");
